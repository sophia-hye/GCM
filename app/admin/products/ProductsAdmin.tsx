...
async function uploadImage(file: File): Promise<string> {
  ...
  const urlRes = await createProductUploadUrl(file.name);
...
function ProductForm({
  ...
  return (
    <form ref={formRef} onSubmit={handleSubmit} className="space-y-3">
      <div>
        <label className="mb-1.5 block text-xs font-semibold text-muted">상품명 *</label>
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        <div>
          ...
          <input name="price" defaultValue={initial?.price != null ? String(initial.price) : ""} inputMode="numeric" placeholder="예: 39000" className={fieldClass} />
        </div>
        <div>
          <label className="mb-1.5 block text-xs font-semibold text-muted">옵션/규격 (선택)</label>
          <input name="duration" defaultValue={initial?.duration ?? ""} placeholder="예: 색상 3종 / S~XL" className={fieldClass} />
        </div>
      </div>
      <div>
...
function ProductRow({ product, index, total }: { product: StoreProduct; index: number; total: number }) {
  const router = useRouter();
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="truncate text-xs text-muted">
            {[formatPrice(product.price), product.duration].filter(Boolean).join(" · ")}
          </p>
        </div>
        <div className="flex shrink-0 items-center gap-1.5">
          <button onClick={() => run(() => moveProduct(product.id, "up"))} disabled={busy || index === 0} className="rounded-lg border border-line px-2 py-1 text-xs disabled:opacity-40 hover:border-court-bright" aria-label="위로">↑</button>
          <button onClick={() => run(() => moveProduct(product.id, "down"))} disabled={busy || index === total - 1} className="rounded-lg border border-line px-2 py-1 text-xs disabled:opacity-40 hover:border-court-bright" aria-label="아래로">↓</button>
          <button onClick={() => run(() => setProductPublished(product.id, !product.published))} disabled={busy} className="rounded-lg border border-line px-3 py-1 text-xs font-semibold hover:border-court-bright">
            {product.published ? "비공개로" : "공개로"}
          </button>
          ...
          <button onClick={() => { if (confirm("이 상품을 삭제할까요?")) run(() => removeProduct(product.id)); }} disabled={busy} className="rounded-lg border border-danger/40 px-3 py-1 text-xs text-danger hover:bg-danger/10">삭제</button>
        </div>
      </div>
      ...
      {editing ? (
        <div className="mt-4 border-t border-line pt-4">
          <ProductForm
            ...
            onSubmit={(values, imagePath) => updateProduct(product.id, { ...values, imagePath })}
          />
        </div>
      ) : null}
    </li>
...
export function ProductsAdmin({ rows }: { rows: StoreProduct[] }) {
    ...
    <div className="space-y-8">
      <div className="rounded-2xl border border-line bg-card p-6">
        <div className="flex items-center justify-between">
          <h2 className="font-display text-lg font-bold">새 상품 등록</h2>
          {!adding ? (
        ...
        {adding ? (
          <div className="mt-5">
            <ProductForm
              ...
              onSubmit={(values, imagePath) => saveProduct({ ...values, imagePath })}
            />
          </div>
        ) : null}
      </div>
      ...
      <div>
        <h2 className="font-display text-lg font-bold">등록된 상품 ({rows.length})</h2>
        {rows.length === 0 ? (
          <p className="mt-3 text-sm text-muted">아직 등록된 상품이 없습니다.</p>
        ) : (
          <ul className="mt-4 space-y-3">
            {rows.map((p, i) => (
              <ProductRow key={p.id} product={p} index={i} total={rows.length} />
            ))}
          </ul>
        )}
      </div>
